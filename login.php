...
define('OAUTH2_CLIENT_ID', '588675894994796545');

define('OAUTH2_CLIENT_SECRET', '_V6gRmNuUmxe-S2QBi1A-OBp2e2I55Pu');
...
if(get('action') == 'login') {
  ...
  $params = array(
    ...
    'redirect_uri' => 'https://falixnodes.host/login',
...
if(session('access_token')) {
  ...
  $user = apiRequest($apiURLBase);
	...
	if( $checkUserInDb->num_rows == 0 ) {
		  ...
		  if( isset($_COOKIE['falixnodes_info']) ) {
		...
		$conn->query("INSERT INTO users (discord_id, pterodactyl_userid, pterodactyl_username, pterodactyl_password, level, register_ip, lastlogin_ip, extra_ram, extra_servers, extra_cores, extra_disk, plan_expiry) VALUES ('" . mysqli_real_escape_string($conn, $user->id) . "', " . $pterodactyl_userid . ", '" . $pterodactyl_username . "', '" . $pterodactyl_password . "', 1, '" . mysqli_real_escape_string($conn, $user_ip) . "', '" . mysqli_real_escape_string($conn, $user_ip) . "', 0, 0, 0, 0, 0)");
