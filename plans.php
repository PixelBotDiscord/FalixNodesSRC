...
if( checklogin() == true ) {
	...
	$user_extra_cores = $conn->query("SELECT * FROM users WHERE discord_id='" . mysqli_real_escape_string($conn, $user->id) . "'")->fetch_assoc()['extra_cores'];
	$user_extra_disk = $conn->query("SELECT * FROM users WHERE discord_id='" . mysqli_real_escape_string($conn, $user->id) . "'")->fetch_assoc()['extra_disk'];
} else {
	$GET_USER_LEVEL = "none";
}
?>